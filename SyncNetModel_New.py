...
tf.disable_v2_behavior()  # Disable TF2 eager execution, use TF1.x graph style

import numpy as np
import utils  # Assuming you have a utils.py file with channel_dropout, fully_connected_layer, predictor_accuracy
...
class SyncNetModel(object):
    ...
    def SyncNetFilters(self, options):
        ...
        t = np.reshape(
            range(-options['Nt']//2, options['Nt']//2),
            [1, options['Nt'], 1, 1]
        )
